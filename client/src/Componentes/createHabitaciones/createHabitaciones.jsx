...
console.log({habitacionData})
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
        <input type="text" name="nombre" placeholder="Nombre" onChange={handleChange}  onBlur={() => handleBlur('nombre')} />
        <p>{touchedFields.nombre && errors.nombre}</p>
        <input type="text" name="imagen" placeholder="Imagen URL" onChange={handleChange}  onBlur={() => handleBlur('imagen')} />
       ...
       <img src={habitacionData.imagen} alt="Vista previa de la imagen" />)}
        ...
        <input type="number" name="precio" placeholder="Precio" onChange={handleChange}  onBlur={() => handleBlur('precio')} />
        <p>{touchedFields.precio && errors.precio}</p>
        <input type="text" name="descripcion" placeholder="Descripcion" onChange={handleChange}  onBlur={() => handleBlur('descripcion')} />
        <p>{touchedFields.descripcion && errors.descripcion}</p>

  <button className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6" type="submit" disabled={submitDisabled}>Crear</button>
</form>
