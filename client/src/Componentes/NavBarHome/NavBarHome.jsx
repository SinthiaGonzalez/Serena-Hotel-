...
function ProfileMenu() {
  ...
  const [namePrueba, setNamePrueba] = useState('');
  useEffect(() => {
  
    const storedName = localStorage.getItem('name');

    if (storedName) {
      // Update the state only if local storage items exist
      setNamePrueba(storedName.replace(/^"|"$/g, ''));
    }
  }, []);
  console.log("aquiiiiiiii", namePrueba)
