...
if (idusuario && fecha_entrada && fecha_salida) {
  try {
    const usuarioExistente = await Usuarioauxiliar.findOne({ where: { iduser: idusuario } });
  ...
  try {
    const body = {
      ...
      back_urls: {
        success:
        `${process.env.URL_NGROK_TEMPORAL}/success`,
        failure:
        `${process.env.URL_NGROK_TEMPORAL}/failure`,
        pending:
        `${process.env.URL_NGROK_TEMPORAL}/pending`,
      },
