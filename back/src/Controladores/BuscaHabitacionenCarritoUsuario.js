...
const buscarHabitacionenCarritoUsuario = async (statusPago,idusuario,fecha_entrada,fecha_salida) => {
    const estado = true;
    ...
    if (statusPago1 === "approved") {
        ...
        if (Carrito && typeof Carrito.findOne === 'function') {
            try {
                ...
                if (fecha_entrada1 && fecha_salida1 && estado && habitacionesIds && usuarioId) {
                    postReservasPagos(fecha_entrada1, fecha_salida1, estado, habitacionesIds, usuarioId);
                };
